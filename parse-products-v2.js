...
tsOutput.forEach(p => {
    ...
    output += "        tagline: `" + p.tagline.replace(/`/g, "'") + "`,\n";
    output += "        description: `" + p.description.replace(/`/g, "'") + "`,\n";
    output += "        price: " + p.price + ",\n";
    output += "        rating: " + p.rating + ",\n";
    output += "        reviews: " + p.reviews + ",\n";
    output += "        image: " + p.image + ",\n"; // No quotes around function call
    output += "        format: '" + p.format + "',\n";
    output += "        needs: " + JSON.stringify(p.needs) + ",\n";
    output += "        portfolio: '" + p.portfolio + "',\n";
    output += "        ingredients: " + JSON.stringify(p.ingredients) + ",\n";
    output += "        benefits: " + JSON.stringify(p.benefits) + ",\n";
    output += "        usage: '" + p.usage + "'\n";
    output += "    },\n";
});
...
const categories = [...new Set(parsed.map(p => p.therapeuticClass.trim()))].sort();
output += "\nexport const CATEGORIES = " + JSON.stringify(categories, null, 4) + ";\n";

fs.writeFileSync('lib/data/products.ts', output);
console.log('Successfully wrote to products.ts');
